...
import { markWhatsappOpened } from '@/lib/services/requests/request-service'
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await markWhatsappOpened(user.userId, parsed.data.id)
    ...
    console.error('[API /api/requests/[id]/whatsapp-opened POST] Error:', error)
    return NextResponse.json(
